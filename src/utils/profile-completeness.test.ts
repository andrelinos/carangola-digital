...
const fullProfile: Partial<ProfileDataProps> = {
  ...
  businessDescription: 'Uma padaria artesanal com mais de 30 anos de tradição em Carangola.',
  ...
  businessAddresses: [{ street: 'Rua das Flores', number: '100', city: 'Carangola', state: 'MG', zipCode: '36801-000', neighborhood: 'Centro' }],
...
describe('calculateProfileCompleteness', () => {
  describe('perfil vazio', () => {
    it('retorna totalScore 0 para perfil sem dados', () => {
      const { totalScore } = calculateProfileCompleteness(emptyProfile as ProfileDataProps)
      expect(totalScore).toBe(0)
    })

    it('retorna todos os items como incompletos', () => {
      const { items } = calculateProfileCompleteness(emptyProfile as ProfileDataProps)
  ...
  describe('perfil completo', () => {
    it('retorna totalScore 100 para perfil 100% preenchido', () => {
      const { totalScore } = calculateProfileCompleteness(fullProfile as ProfileDataProps)
      expect(totalScore).toBe(100)
    })

    it('retorna todos os items como completos', () => {
      const { items } = calculateProfileCompleteness(fullProfile as ProfileDataProps)
  ...
  describe('checagem individual de itens', () => {
    it('marca "description" completo quando tem mais de 20 chars', () => {
      const profile = { ...emptyProfile, businessDescription: 'Descrição com mais de vinte caracteres aqui.' }
      const { items } = calculateProfileCompleteness(profile as ProfileDataProps)
    ...
    it('marca "logo" completo quando há logoImageUrl', () => {
      const profile = { ...emptyProfile, logoImageUrl: 'https://cdn.exemplo.com/logo.png' }
      const { items } = calculateProfileCompleteness(profile as ProfileDataProps)
    ...
    it('marca "phones" completo quando há pelo menos 1 telefone', () => {
      const profile = { ...emptyProfile, businessPhones: [{ phone: '32988887777', label: 'Fixo' }] }
      const { items } = calculateProfileCompleteness(profile as ProfileDataProps)
    ...
    it('marca "social" completo quando há pelo menos 1 rede social preenchida', () => {
      const profile = { ...emptyProfile, socialMedias: { instagram: 'minha_loja' } }
      const { items } = calculateProfileCompleteness(profile as ProfileDataProps)
    ...
    it('marca "social" incompleto quando todas as redes são vazias', () => {
      const profile = { ...emptyProfile, socialMedias: { instagram: '', facebook: '' } }
      const { items } = calculateProfileCompleteness(profile as ProfileDataProps)
      const item = items.find(i => i.id === 'social')
      expect(item?.isComplete).toBe(false)
    })
    ...
    it('soma corretamente os scores dos itens completos', () => {
      ...
      const { totalScore } = calculateProfileCompleteness(profile as ProfileDataProps)
