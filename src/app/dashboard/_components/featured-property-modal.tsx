...
import { setFeaturedPropertyStatus } from '@/actions/properties/set-featured-property-status.action'
...
interface FeaturedPropertyModalProps {
  open: boolean
...
export function FeaturedPropertyModal({
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await setFeaturedPropertyStatus({
        propertyId,
        ownerId,
        ...
        <DialogHeader className="space-y-4">
          ...
          <div className="text-center">
            <DialogTitle className="font-black text-2xl text-slate-900 uppercase tracking-tighter">
              {isActivating ? 'Destacar Imóvel' : 'Remover Destaque'}
            </DialogTitle>
            <p className="mt-1 font-medium text-slate-500 text-sm">
              {isActivating
                ? 'O imóvel aparecerá na seção de destaques da plataforma.'
                : 'O imóvel deixará de aparecer na seção de destaques.'}
            </p>
          </div>
        </DialogHeader>
