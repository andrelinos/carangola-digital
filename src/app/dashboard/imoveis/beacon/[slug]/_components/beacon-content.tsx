...
import { ArrowLeft, MapPin, Radio, Home } from 'lucide-react'
  ...
  const toggle = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        toast.success(result.message || (nextState ? 'Imóvel em destaque no mapa!' : 'Destaque removido.'))
      }
    } catch (error) {
      // Rollback
      setIsActive(isActive)
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 font-medium text-slate-500 text-sm dark:text-slate-400">
              Escolha se este imóvel deve aparecer para clientes próximos que buscam
              pelo mapa em tempo real.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900">
              ...
              <p className="text-slate-500 text-xs leading-relaxed dark:text-slate-400">
                ...
                pesquisando na sua região. Use para atrair visitas
                imediatas.
              </p>
            </div>
          </div>
