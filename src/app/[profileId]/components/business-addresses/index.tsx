...
interface AddressItemProps {
  ...
  neighborhood: string
  cep: string
  latitude: number | null // Permitir nulo para maior segurança
  longitude: number | null // Permitir nulo para maior segurança
}
...
export function BusinessAddresses({ profileData, isOwner, isUserAuth }: Props) {
  ...
  // REMOVIDO: A variável `mapsLink` e a função `setMapsLink` no escopo do componente
  // não são necessárias, pois cada item do loop terá seu próprio link.
        ...
        {businessAddresses?.map((item, index) => {
          ...
          return (
            item.address && (
              <div
                key={String(index)}
                className="flex w-full flex-col gap-2 p-4 hover:bg-zinc-50"
              >
